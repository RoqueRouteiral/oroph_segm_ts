...
"""
Adapted code from: https://github.com/oikosohn
The code is a pytorch adaptation of the official code by the Unified Focal loss function:
https://github.com/mlyg/unified-focal-loss
We modified the previous code to work in non hot encoder outputs.
"""
...
class AsymmetricFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        back_pred = torch.ones_like(y_pred) - y_pred
        back_true = torch.ones_like(y_true) - y_true
        back_cross_entropy = -back_true * torch.log(back_pred)
        fore_cross_entropy = -y_true * torch.log(y_pred)
        
	    #calculate losses separately for each class, only suppressing background class
        back_ce = torch.pow(1 - back_pred, self.gamma) * back_cross_entropy
        back_ce =  (1 - self.delta) * back_ce

        fore_ce = fore_cross_entropy
        fore_ce = self.delta * fore_ce
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        back_true = torch.ones_like(y_true) - y_true
        # Calculate true positives (tp), false negatives (fn) and false positives (fp)     
        fore_tp = torch.sum(y_true * y_pred)
        fore_fn = torch.sum(y_true * (1-y_pred))
        fore_fp = torch.sum((1-y_true) * y_pred)
        fore_dice_class = (fore_tp + self.epsilon)/(fore_tp + self.delta*fore_fn + (1-self.delta)*fore_fp + self.epsilon)
        
        back_tp = torch.sum(back_true * back_pred)
        back_fn = torch.sum(back_true * (1-back_pred))
        back_fp = torch.sum((1-back_true) * back_pred)
        back_dice_class = (back_tp + self.epsilon)/(back_tp + self.delta*back_fn + (1-self.delta)*back_fp + self.epsilon)
        
        # print('Dice_TV,0: ', back_dice_class)
        # print('Dice_TV,1: ', fore_dice_class)
        #calculate losses separately for each class, only enhancing foreground class
        back_dice = (1-back_dice_class) 
        fore_dice = (1-fore_dice_class) * torch.pow(1-fore_dice_class, -self.gamma) 
...
class AsymmetricUnifiedFocalLoss(nn.Module):
    ...
    epsilon : float, optional
        clip values to prevent division by zero error
    """
    def __init__(self, weight=0.5, delta=0.6, gamma=0.8): #(self, weight=0.5, delta=0.6, gamma=0.2)
