...
def get_loaders_xnet_linear(cf, phase='train'):
...
class HeadyNeckDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        coor = np.where(gt_np)
        gt_border= np.array([np.min(coor[0]),np.max(coor[0])+1,np.min(coor[1]),np.max(coor[1])+1,np.min(coor[2]),np.max(coor[2])+1])
        ...
        gt_box = torch.from_numpy(gt_border.copy().astype(float)).unsqueeze(0).float()
