...
def get_loaders_boxes_inference(cf, phase='train'):
...
class HeadyNeckDataset(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        box_name = os.path.join(self.cf['box_path'], self.indices[idx], 'box.npy')
        ...
        box_np = np.load(box_name)>0 #check this  

        if self.cf['box_prediction']:
            gt_dist = ndimage.distance_transform_edt(box_np==0)
            gt_dist = gt_dist<1       
        else:
